...
message_before = 'Intro Web Science';
message_after = ""
...
class Server:
 ...
 def __init__(self, port = 8080):
     ...
     self.port = port
     self.www_dir = '/home/jp/ass5' # Directory where webpage files are stored
 ...
 def long_pooling(self, conn):
     global message_after, message_before
     message_after = input("Your Message Here ")
     if not message_after == '' and not message_after == message_before:
        message_before = message_after
     else:
        message_after = message_before

     response_headers = self._gen_headers( 200)
     response_content = response_headers.encode()
     response_content += message_after.encode()
 ...
 def _wait_for_connections(self):
     ...
     while True:
         ...
         if (request_method == 'GET') | (request_method == 'HEAD'):
             ...
             if file_requested == '/server_message':
                self.long_pooling(conn)
             else:
                 if (file_requested == '/'):  # in case no file is specified by the browser
                     file_requested = '/webclient.html' # load index.html by default
